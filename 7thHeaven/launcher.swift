...
let FF7_GAME_DIR = "FINAL FANTASY VII"
...
func downloadInstaller() -> Bool {
    ...
    DispatchQueue.main.async {
        showStatusMessage(message: "Downloading 7th Heaven installer. This may take a few minutes...", style: .informational)
    }
...
func findFF7Path() -> (path: String, installDir: String, gameID: String, libraryPath: String)? {
    let ff7GameIDs = ["39140", "3837340"]  // FF7 original and remaster
    ...
    // Parse Steam library folders and check for FF7 game IDs
    if FileManager.default.fileExists(atPath: steamConfig.path),
       let content = try? String(contentsOf: steamConfig, encoding: .utf8) {
        ...
        for line in lines {
            ...
            // Check for FF7 game IDs in the apps section
                    ...
                    if line.contains("\"\(gameID)\"") {
                        ...
                        if let installDir = parseInstallDirFromAppManifest(manifestPath) {
                            ...
                            if FileManager.default.fileExists(atPath: candidate) {
                                log("Found FF7 (Game ID: \(gameID)) at: \(candidate) [installdir=\(installDir)]")
                                return (candidate, installDir, gameID, libraryPath)
                            }
                        }

                        let fallbackCandidate = "\(libraryPath)/steamapps/common/\(FF7_GAME_DIR)"
                        if FileManager.default.fileExists(atPath: fallbackCandidate) {
                            log("Found FF7 (Game ID: \(gameID)) at fallback path: \(fallbackCandidate)")
                            return (fallbackCandidate, FF7_GAME_DIR, gameID, libraryPath)
                        }
                    }
                }
            }
            ...
            for gameID in ff7GameIDs {
                ...
                if FileManager.default.fileExists(atPath: manifestPath) {
                    if let installDir = parseInstallDirFromAppManifest(manifestPath) {
                        let candidate = "\(libraryPath)/steamapps/common/\(installDir)"
                        if FileManager.default.fileExists(atPath: candidate) {
                            log("Found FF7 (Game ID: \(gameID) via manifest) at: \(candidate) [installdir=\(installDir)]")
                            return (candidate, installDir, gameID, libraryPath)
                        }
                    }
                    ...
                    let fallbackCandidate = "\(libraryPath)/steamapps/common/\(FF7_GAME_DIR)"
                    if FileManager.default.fileExists(atPath: fallbackCandidate) {
                        log("Found FF7 (Game ID: \(gameID) via manifest fallback) at: \(fallbackCandidate)")
                        return (fallbackCandidate, FF7_GAME_DIR, gameID, libraryPath)
                    }
                }
            }
    ...
    let fallbacks = [
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/Steam/steamapps/common/\(FF7_GAME_DIR)").path,
        "/Volumes/SteamLibrary/steamapps/common/\(FF7_GAME_DIR)",
...
func isSteamFF7Install(path: String, gameID: String, libraryPath: String) -> Bool {
...
func ensureSteamUserDirectoriesIfNeeded(isSteamInstall: Bool) {
    ...
    let documentsPath = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent("Documents/Square Enix/FINAL FANTASY VII Steam/user_12345678")
...
// MARK: - Copy FF7 Game Into Wine Prefix

func copyFF7IntoWinePrefix(from sourcePath: String, installDir: String) -> String? {
    ...
    guard FileManager.default.fileExists(atPath: sourceURL.path) else {
        log("Source FF7 path does not exist: \(sourceURL.path)")
        return nil
    }
    ...
    if FileManager.default.fileExists(atPath: destinationURL.path) {
        if let existingEntries = try? FileManager.default.contentsOfDirectory(atPath: destinationURL.path),
           !existingEntries.isEmpty {
            log("FF7 already present in Wine prefix at: \(destinationURL.path), skipping copy")
    ...
    showStatusMessage(message: "Copying Final Fantasy VII into Wine prefix. This may take a while...", style: .informational)

    // Use native Swift recursive copy with cleanup
    if recursivelyCopyFiles(from: sourceURL, to: destinationURL) {
        log("FF7 copied to Wine prefix at: \(destinationURL.path)")
        return destinationURL.path
    }

    log("Failed to copy FF7 using recursive copy")
    try? FileManager.default.removeItem(at: destinationURL)
    return nil
}
...
final class StatusWindow {
    ...
    init() {
        ...
        self.window.title = "7th Heaven - Status"
        self.window.center()
        ...
        self.textView.string = "INFO: Launching 7th Heaven...\n"
...
func runMain() {
    ...
    if FileManager.default.fileExists(atPath: targetExe.path) {
        ...
        showStatusMessage(message: "Running 7th Heaven installer. Please wait...", style: .informational)
        ...
        if let customInstaller = promptForCustomFF7Installer() {
            ...
            showStatusMessage(message: "Running FF7 installer. Please follow the on-screen instructions.", style: .informational)
    ...
    // Find FF7 installation and copy it to the expected Steam path in Wine prefix
    log("Locating Final Fantasy VII installation...")
    guard let ff7Install = findFF7Path() else {
        showErrorAndExit("Could not locate Final Fantasy VII in your Steam library. Please ensure it is installed via Steam.")
    }

    let steamInstallDetected = isSteamFF7Install(path: ff7Install.path, gameID: ff7Install.gameID, libraryPath: ff7Install.libraryPath)
    ensureSteamUserDirectoriesIfNeeded(isSteamInstall: steamInstallDetected)

    guard copyFF7IntoWinePrefix(from: ff7Install.path, installDir: ff7Install.installDir) != nil else {
        showErrorAndExit("Failed to copy Final Fantasy VII into the Wine prefix. Please check permissions and free disk space.")
    }
